...
import { setSettings } from "+/redux/slices/appSlice"
...
export function AudioPanel({ className }: AudioPanelProps) {
  ...
  const { speak, stopSpeaking, voices, isSpeaking } = useSpeaker({ enabled: true })
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className || ""}`}>
      <div className="flex flex-1 items-center gap-2 rounded-xl border border-ink-100 bg-white px-2.5 py-1.5 text-xs shadow-inner">
        <button
          type="button"
          onClick={togglePreview}
          className="rounded-lg border border-ink-100 bg-ink-50 px-2 py-1 font-semibold text-ink-800 shadow-inner"
        ...
        <input
          ...
          className="w-56 bg-transparent text-ink-900 outline-none"
        />
        <button
          type="button"
          onClick={togglePause}
          disabled={!systemSpeaking}
          className="rounded-lg border border-ink-100 bg-white px-2 py-1 font-semibold text-ink-700 shadow-inner disabled:opacity-40"
      ...
      <div className="relative" ref={configRef}>
        ...
        {showAudioConfig && (
          <div className="absolute right-0 top-full z-20 mt-2 w-[320px] rounded-xl border border-ink-100 bg-ink-50/95 px-3 py-3 text-xs text-ink-700 shadow-soft backdrop-blur">
